...
void GDRetro::set_texture(const Ref<Texture2D> &p_texture) {
  ...
  godot::UtilityFunctions::print("set_texture: ");
  if (p_texture == texture) {
      return;
  }

  static const StringName changed_signal("changed"); // CoreStringNames::get_singleton()->changed
  static const StringName texture_changed_signal("texture_changed"); // SceneStringNames::get_singleton()->texture_changed

  if (texture.is_valid()) {
      texture->disconnect(changed_signal, callable_mp(this, &GDRetro::_texture_changed));
  }

  texture = p_texture;

  if (texture.is_valid()) {
      texture->connect(changed_signal, callable_mp(this, &GDRetro::_texture_changed));
  }

  queue_redraw();
  emit_signal(texture_changed_signal);
